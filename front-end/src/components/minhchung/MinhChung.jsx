...
const MinhChung = () => {
  ...
  const fetchCategories = async () => {
    try {
      ...
      setErrorMessage("Không thể tải danh sách phân loại");
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    if (!selectedCategory || files.length === 0) {
      setErrorMessage("Vui lòng chọn phân loại và tải lên ít nhất một file");
      return;
    }
    ...
    try {
      ...
      const evidenceData = {
        studentId: userDetails.studentId || userDetails.uid, // Sử dụng mã học sinh thực tế, fallback về UID
        uid: userDetails.uid, // Thêm UID để đảm bảo có thể tìm kiếm
        firstName: userDetails.firstName || "Không xác định", // Thêm firstName để tương thích
        ...
        categoryPoints: selectedRule.points,
        status: "pending",
              ...
              <div>
                ...
                <ul className="space-y-1 text-sm" style={{ color: "#064232" }}>
                  ...
                  <li className="flex items-start gap-2">
                    <span className="text-xs mt-1">•</span>
                    <span>
                      Thêm ghi chú mô tả chi tiết về minh chứng (tùy chọn)
                    </span>
                  </li>
                  <li className="flex items-start gap-2">
                    <span className="text-xs mt-1">•</span>
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div>
                  ...
                  <select
                    ...
                    required
                  >
                    <option value="">Chọn phân loại điểm cộng</option>
                    {categories.map((category) => (
                      <option key={category.id} value={category.id}>
                        {category.name} (+{category.points} điểm)
                      </option>
                    ))}
                  </select>
                </div>
                ...
                <div>
                  ...
                  <textarea
                    ...
                    placeholder="Mô tả thêm về minh chứng của bạn..."
                  />
                </div>
