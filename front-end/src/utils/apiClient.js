...
const API_BASE_URL =
  import.meta.env.VITE_RAG_API_URL || "http://localhost:5002";
...
export class ApiClient {
  ...
  async testAuth(authData) {
    const { token, userId, onAuthError } = authData;

    const options = {
      method: "POST",
      headers: this.createHeaders(token, userId),
      body: JSON.stringify({
...
export const createAuthData = (user, authToken, getAuthToken) => ({
  token: authToken,
  userId: user?.uid,
  onAuthError: async () => {
    try {
